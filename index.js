...
import {
  ...
  getUser,
  getUserTradeByMsg,
  updateUserAutoBuySetting,
  updateUserAutoSellSetting,
  updateUserBuyWithSetting,
  updateUserReferrals,
  updateUserSellAtSetting,
  updateUserTrade,
  updateUserTrades,
  updateUserWallet,
} from "./src/db/db.js";
  ...
  try {
    ...
    if (is_user) {
      ...
      if (approved) {
        ...
        if (result[2]) {
          await updateUserTrade(userId, trade.bought_at, timestamp, pnl[0]);
        }
      }
      ...
      if (bought[0]) {
        await ctx.editMessageText(
          `<i>📈 Trade successfully excecuted.</i>\n\n<i>Token : <code>${
            trades[0].token
          }</code></i>\n\n<i>Price: ${Number(tokenInfo[4]).toFixed(
            6
          )} TRX</i>\n\n<i>Amount : ${trades[0].base_amount.toFixed(
            2
          )} TRX</i>\n\n<i>Bought : ${toDecimals(
            trades[0].quote_amount,
            tokenInfo[0],
            false
          ).toFixed(2)} ${
            trades[0].quote
          }</i>\n\n<i>pNl : 0</i>\n\n<i>Time : ${new Date(
            trades[0].bought_at * 1000
          )}</i>`,
          {
            parse_mode: "HTML",
...
bot.action("set_sell_x", async (ctx) => {
  try {
    await ctx.replyWithHTML(
      "<i>🔁 Reply with the percent of price increase you want to auto sell at.</i>"
    );
  } catch (err) {
    await ctx.replyWithHTML(
      `<b>🚫 An error just ocurred. Sorry for the Inconveniences.</b>`
  ...
  try {
    ...
    if (bought[0]) {
      await ctx.editMessageText(
        `<i>📈 Trade successfully excecuted.</i>\n\n<i>Token : <code>${
          ctx.session.token
        }</code></i>\n\n<i>Price: ${Number(bought[1][4]).toFixed(
          6
        )} TRX</i>\n\n<i>Amount : ${(Number(bought[2]) / 1_000_000).toFixed(
          2
        )} TRX</i>\n\n<i>Bought : ${toDecimals(
          bought[3],
          bought[1][0],
    ...
    await ctx.replyWithHTML(
      `<b>🚫 Make sure you have enough TRX/Energy/Bandwidth for this transaction.</b>\n\n<i>Please we do not support SunPump meme coins currently(coming soon).</i>`
    );
    console.log(err);
  }
});
...
bot.action("buy_x", async (ctx) => {
  try {
    await ctx.replyWithHTML(
      "<i>🔁 Reply with the amount of TRX you want to buy with.</i>"
    );
  } catch (err) {
    await ctx.replyWithHTML(
      `<b>🚫 An error just ocurred. Sorry for the Inconveniences.</b>`
      ...
      if (trades.length > 0) {
        ...
        trades.forEach(async (trade, i) => {
          ...
          text += `<b>${i + 1}.)</b><i>Token : <code>${
            ...
            tokenInfo[0],
            false
          ).toFixed(2)} ${trade.quote}</i>\n\n<i>Time : ${new Date(
            Number(trade.bought_at) * 1000
          )}</i>\n\n`;
        });
...
bot.action("sell_100", async (ctx) => {
  try {
    ...
    if (is_user && !is_user[0].settings.auto_sell) {
      ...
      await ctx.replyWithHTML(
        "<i>🔁 Reply with the token address you want to sell.</i>"
      );
    } else {
...
bot.action("sell_x", async (ctx) => {
  try {
    ...
    if (is_user && !is_user[0].settings.auto_sell) {
      await ctx.replyWithHTML(
        "<i>🔁 Reply with the percentage of the tokens you want to sell.</i>"
      );
    } else {
      await ctx.reply("Auto Sell is Enabled. You must disable it to continue.");
    }
      ...
      if (isAddress) {
        if (is_user) {
          if (
            ...
            "reply_to_message" in ctx.message &&
            isFinite(ctx.message.reply_to_message.text)
          ) {
            const result = await withdraw(
              is_user[0].pubKey,
              is_user[0].secKey,
            ...
            "reply_to_message" in ctx.message &&
            ctx.message.reply_to_message.text ==
              "🔁 Reply with the token address you want to sell."
          ) {
            ...
            if (sold[0]) {
              await ctx.editMessageText(
                `<i>📈 Sold successfully excecuted.</i>\n\n<i>Token : ${
                  ctx.message.text
                }</i>\n\n<i>Price: ${Number(sold[1][4]).toFixed(
                  6
                )} TRX</i>\n\n<i>Amount : ${(
                  Number(sold[2]) / 1_000_000
                ).toFixed(2)} TRX</i>\n\n<i>Sold : ${toDecimals(
                  sold[3],
                  sold[1][0],
                  false
                ).toFixed(2)} ${sold[1][1]}</i>\n\n<i>Time : ${new Date(
                  sold[4] * 1000
                )}</i>`,
                {
                  parse_mode: "HTML",
            ...
            if (is_user[0].settings.auto_buy) {
              ...
              const bought = await buyToken(
                ctx.chat.id,
                ctx.message.text,
                Number(is_user[0].settings.buy_with),
                msg.message_id
              );
              ...
              await ctx.replyWithHTML(
                `<b>💎 ${tokenInfo[5]} | ${
                  ...
                  ctx.message.text
                }</code>\n\n<b>💵 Price:</b><i>${Number(tokenInfo[4]).toFixed(
                  6
                )} TRX</i>\n\n<b>💳 Wallet Balance:</b><i>${(
                  Number(balance) / 1_000_000
                ).toFixed(
                  2
...
bot.on("message", async (ctx) => {
  try {
    if ("reply_to_message" in ctx.message) {
      ...
      if (
        text == "🔁 Reply with the percentage of the tokens you want to sell."
      ) {
      ...
      if (text == "🔁 Reply with the amount of TRX you want to withdraw.") {
        ...
        await ctx.sendMessage(
          `🖕 Reply to the above text with the destination address.`
        );
      }
      ...
      if (
        text == "🔁 Reply with the amount of TRX you want to auto buy with."
      ) {
        console.log(ctx.message.text);
        const is_user = await isUser(ctx.chat.id);
        console.log(is_user);
        ...
        if (is_user[1]) {
          if (is_user[0].settings.auto_buy) {
            await updateUserBuyWithSetting(
              ctx.chat.id,
              Number(ctx.message.text)
            );

            await ctx.reply(
              `🟢 Auto Buy is set to buy with ${ctx.message.text} TRX.`
            );
          }
        }
      }
      ...
      if (
        text ==
        "🔁 Reply with the percent of price increase you want to auto sell at."
      ) {
        console.log(ctx.message.text);
        const is_user = await isUser(ctx.chat.id);
        console.log(is_user);
        ...
        if (is_user[1]) {
          if (is_user[0].settings.auto_sell) {
            await updateUserSellAtSetting(
              ctx.chat.id,
              Number(ctx.message.text)
            );

            await ctx.reply(
              `🟢 Auto Sell is now set at ${ctx.message.text}% increase in price.`
            );
          }
        }
      }
